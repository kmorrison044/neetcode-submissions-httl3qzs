class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        # Keep track of a shortest list that finds the shortest path
        # for each node in the graph.
        shortest = [float('inf') for node in range(n + 1)]
        ...
        # We want the longest time to get to all nodes. This is the max value in the shortest
        # list if all nodes were visited. If not, then return -1 as per the requirement.
        return max(shortest) if len(visit) == n else -1